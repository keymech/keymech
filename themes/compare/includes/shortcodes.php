...
class Compare_Shortcodes{
	...
	function add_buttons( $plugin_array ) {
	    $plugin_array['compare'] = get_template_directory_uri() . '/js/shortcodes.js';
	    return $plugin_array;
	}

	function register_buttons( $buttons ) {
	    array_push( $buttons, 'comparegrid', 'compareelements' ); 
	    return $buttons;
	}

	function shortcode_options(){
		if( $_POST['shortcode'] == 'row' || $_POST['shortcode'] == 'content' ){
			echo '';
		}
		else{
			$shortcode = 'compare_'.$_POST['shortcode'].'_params';
	...
	function render_options( $fields ){
				...
				switch ( $field['type'] ){
					...
					case 'attach_image' :
						$fields_html .= '<div class="shortcode-image-holder"></div><div class="clearfix"></div>
										<a href="javascript:;" class="shortcode-add-image button">'.__( 'Add Image', 'compare' ).'</a>
					...
					case 'attach_images' :
						$fields_html .= '<div class="shortcode-images-holder"></div><div class="clearfix"></div>
										<a href="javascript:;" class="shortcode-add-images button">'.__( 'Add Images', 'compare' ).'</a>
		...
		echo  $fields_html.'<a href="javascript:;" class="shortcode-save-options button">'.__( 'Insert', 'compare' ).'</a>';
		die();
